...
def int_functional(animal, mat_types):
    print("Computing intersection with functional connectome")
    ...
    for prunning_name, prunning in mat_types.items():
        ...
        for seed in mats.keys():
            intersection_functional[prunning_name][seed]=mats[seed].astype("bool").multiply(A_functional.astype("bool")).nnz
        print(f"Done with {prunning_name}")
    path_out=f"{root}/mat_intersection/intersections_with_funct_{animal}.pkl"
    with open(path_out, "wb") as f:
        pickle.dump(intersection_functional, f)
    print(f"Done with intersections with functional for {animal}")

   
def int_controls(animal, mat_types, reduced=False):
    prunning_types=list(mat_types.keys())
    print("Computing intersection across controls")
    ...
    for i, j in itertools.combinations_with_replacement(prunning_types, 2):
        ...
        if i==j:
            l=0
            for s in seeds:
                intersections[(i,j)][(0,s)]=mats_i[l].astype("bool").multiply(mats_i[s].astype("bool")).nnz
        if reduced: 
            if i!=j:
                ...
                l=0
                for s in seeds:
                    intersections[(i,j)][(l,s)]=mats_i[l].astype("bool").multiply(mats_j[s].astype("bool")).nnz
        else: 
            if i!=j:
                path_j=f"{root}/mat_intersection/{mat_types[j].__name__}_multiple_{animal}.pkl"
                with open(path_i, "rb") as f:
    ...
    path_out=f"{root}/mat_intersection/intersections_across_controls_{animal}.pkl"
    with open(path_out, "wb") as f:
        pickle.dump(intersections, f)
    print(f"Done with intersections across controls {animal}")
